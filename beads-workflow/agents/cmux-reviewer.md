...
if _lib is None:
    print('Phase 2 metrics skipped: plugin lib not found', file=sys.stderr); sys.exit(0)
...
- **Write the fix prompt once, at its natural length.** Length follows the number and depth of findings — one finding is short, five with root-cause detail is longer. Do NOT draft, count, and rewrite to hit a length target. Match complexity, do not pad, do not trim.
