...
**When it runs**: When CLAUDE.md or `.claude/rules/*.md` files are loaded into context.

**Purpose**: React to instruction loading events, validate instructions, or inject additional context.

**Blocking capability**: Cannot block (informational only).

**Common use cases**:
- Log which instruction files were loaded
- Validate instruction file integrity
- Inject environment-specific overrides after instructions load
- Track instruction file changes across sessions
    ...
    "InstructionsLoaded": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo 'Instructions loaded' >> /tmp/claude-hooks.log"
          }
        ]
      }
    ]
...
## Hook Event Data Fields

Since v2.1.69, hook events include additional fields:

| Field | Available in | Description |
|-------|-------------|-------------|
| `agent_id` | All events (subagents only) | Unique ID of the subagent that triggered the event |
| `agent_type` | All events (subagents and `--agent`) | Type identifier of the agent |
| `worktree` | Status line hooks | Object with `name`, `path`, `branch`, and original repo directory |

These fields enable hooks to distinguish between main agent and subagent activity, useful for filtering or routing hook behavior.
