...
- You hit EADDRINUSE and need to resolve a port conflict between dev servers
- You want a stable, named URL (e.g. myapp.localhost) instead of remembering port numbers
- You are running multiple dev servers (monorepo, frontend + backend) and need them isolated
- You need multi-agent or worktree isolation with parallel dev server instances

## Do NOT

- Do NOT add portless as a project dependency — it's a global dev tool
- Do NOT use for production deployments — portless is development-only
...
When multiple agents or git worktrees need their own dev servers, use a namespace prefix:

```bash
# Agent 1
portless agent1-api bun run --watch src/index.ts  # http://agent1-api.localhost:1355
API_URL=http://agent1-api.localhost:1355 portless agent1 bun run dev  # http://agent1.localhost:1355

# Agent 2 (parallel, zero conflicts)
portless agent2-api bun run --watch src/index.ts  # http://agent2-api.localhost:1355
API_URL=http://agent2-api.localhost:1355 portless agent2 bun run dev  # http://agent2.localhost:1355
```

Convention: `<ns>-api` for backend, `<ns>` for frontend. Set `API_URL` so each frontend proxies to its own backend.

## Gotchas

- **Binary not in PATH**: `portless myapp next dev` fails if `next` is not a global binary. Use `portless myapp bun run dev` (or `npx`, `pnpm exec`) to resolve via node_modules/.bin.
- **API_URL must match namespace**: If your frontend proxies `/api/*` to a backend, set `API_URL` to the namespaced backend URL, not a hardcoded port.
