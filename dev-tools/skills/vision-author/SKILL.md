...
```bash
TENSE_GATE="scripts/tense-gate.py"
if [[ ! -f "$TENSE_GATE" ]]; then
  echo "ERROR: tense-gate not found at $TENSE_GATE"
  echo "Fix: install the tense-gate tool via CCP-2q2 or run:"
  echo "  cp path/to/tense-gate.py scripts/tense-gate.py"
  echo "The /vision-author skill requires tense-gate for section validation."
  exit 2
fi
```
...
result = check_conformance(Path("docs/vision.md"))
```
...
spec = importlib.util.spec_from_file_location("tense_gate", "scripts/tense-gate.py")
...
Create `docs/adr/0000-vision-initial.md`:
...
ADR content (substitute `{project_name}` from the vision statement or directory name,
