...
- *(CCP-xtba)* **Wave-orchestrator double-dispatch prevention and session-close ownership check** — adds duplicate-dispatch guard to wave-dispatch.py: refuses to create a second cmux pane if worktree bead-X exists with uncommitted changes OR a claude process with `--worktree bead-X` is already running; emits structured warning with offending PID(s). Also adds ownership check to phase-b-close-beads.sh: skips bead close if `CCP_SESSION_ID` is set and assignee differs from current session, preventing cosmetic close-reason corruption from out-of-session close attempts. Fixes double-dispatch regression observed on CCP-top (two PIDs 40500 and 43610 running concurrently on same worktree, resulting in conflicting commits 6dc3bcd and 90d6b90). Regression test: dispatching an already-running bead returns structured 'already-running' result instead of creating a second pane. 45 orchestrator tests passing.
- *(CCP-zodj)* **Daily-brief open-brain MCP client replaced with official SDK** — replaces homegrown `_OBClient` (httpx JSON-RPC) in `query-sources.py` and `_async_save_memory()` in `orchestrate-brief.py` with the official `mcp` Python SDK (`mcp>=1.11`). The SDK handles MCP Streamable-HTTP session lifecycle (initialize handshake, session-id negotiation, SSE parsing) transparently. Fixes 400 errors caused by missing initialize handshake. Adds `isError` check to detect SDK tool-level failures, and walks full `ExceptionGroup` chain for actionable 401 auth warnings. ADR-0001 documents the Path B decision. 113 tests passing (2 skipped — integration gated on `OB_API_KEY`). Live smoke test: brief for 2026-04-24 generated with 10 sessions, 1 decision, no MCP warnings.
